<template>
  <div>
    ...
    <div>
      <a-row type="flex" justify="center">
        <a-col :span="12" style="padding: 10px">
          ...
          <a-button @click="addParams" style="margin: auto; display: block"
            >新增搜索名称</a-button
          >
        </a-col>
        <a-col :span="12" style="padding: 10px">
          <a-input v-model="setparamEn" style="margin-bottom: 5px"></a-input>
          <a-button @click="addParamConnet" style="margin: auto; display: block"
            >新增搜索连接</a-button
          >
        </a-col>
      </a-row>
    ...
    <a-list
      ...
      :dataSource="propsConnectsNow"
    >
      <a-list-item slot="renderItem" slot-scope="item">
        <a-list-item-meta>
          <div class="m-l-10 font-16" slot="title">
            {{ item.propsEn.props }}
          </div>
        </a-list-item-meta>
    ...
    <a-list
      style="background-color: white"
      item-layout="horizontal"
      :dataSource="propsConnectsRecent"
    >
      <a-list-item slot="renderItem" slot-scope="item">
        <a-list-item-meta>
          <div class="m-l-10 font-16" slot="title">
            变量意思：{{ item.prop.content }}； 变量名：{{ item.propsEn.props }}
          </div>
        </a-list-item-meta>
...
export default {
  ...
  apollo: {
    ...
    newPropsConnect: {
      // 是否存在旧连接
      query: gql`
        query($propsEnId: uuid!, $propsId: uuid!) {
          propskit_propsConnect(
            where: {
              propsId: { _eq: $propsId }
              propsEnId: { _eq: $propsEnId }
            }
          ) {
            referTimes
            created_at
      ...
      skip() {
        let bool1 =
          this.getparam &&
          this.getparam.hasOwnProperty(length) &&
          this.getparam.length > 0;
        let bool2 =
          this.getparamEn &&
          this.getparamEn.hasOwnProperty(length) &&
          this.getparamEn.length > 0;
    ...
    propsConnectsNow: {
      ...
      query: gql`
        query($content: String!) {
          propskit_propsConnect(
            limit: 6
            order_by: { referTimes: asc }
            where: {
              prop: {
                content: { _like: $content }
                _or: { content: { _similar: $content } }
              }
            }
          ) {
            referTimes
  ...
  methods: {
    ...
    putNewconnect: async function (propsId, propsEnId) {
      let data = await this.$apollo
        .mutate({
          mutation: gql`
            mutation($propsEnId: uuid!, $propsId: uuid!) {
              insert_propskit_propsConnect(
                objects: { propsEnId: $propsEnId, propsId: $propsId }
              ) {
                returning {
                  id
                  created_at
